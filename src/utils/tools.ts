 ...
 */
export function parseBankNoInfo(bankNo: string) {
  const cardTypeMap = {
    DC: "储蓄卡",
    CC: "信用卡",
    SCC: "准贷记卡",
  ...
  const bankCardList = [
    ...
    },
    {
      ...
      patterns: [
        ...
        {
          reg: /^(405512|601428|622258|622259)\d{11}$/g,
          cardType: "DC",
        },
        {
  ...
  for (let i = 0, len = bankCardList.length; i < len; i++) {
    ...
    for (let j = 0, jLen = patterns.length; j < jLen; j++) {
      ...
      if (new RegExp(pattern.reg).test(bankNo)) {
        const info: any = extend(bankcard, pattern);
 ...
 * @returns
 */
export function randomBankNo(bank: string) {
  let prefix = "";
  switch (bank) {
    case "0102": {
      prefix = "622202";
